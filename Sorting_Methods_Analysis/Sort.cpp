...
void printResult(int sortType, unsigned long int length, double ms){
        ...
        if (sortType == 1){
            ...
            file_name="TreeSort.txt";
        }
        else if (sortType == 4){
            file_name="ShellSort.txt";
        }
        else if (sortType == 5){
            file_name="HooraSort.txt";
        }
...
int main (void){
	...
	//  Создаем переменные
	int i=1,d;					// Счетчик количества файлов
	unsigned long int j;		// Переменная для получения массива чисел
	int sortType;				// Способ сортировки
	FILE *file;					// Файл
	double time;                // Переменная времени
	const int maxnum = 100000;	// Переменная не нужна, но при ее удалении, почемуто, перестают считываться файлы...Магия.
	double sum = 0;				// Файл суммы милисекунд
	double avgMs = 0;		    // Среднее количество милисекунд потраченное на выполнения сортировки

	// Информирование о способах сортировки
	sortInfo();
	...
	while (true){
		...
		int array[100000];									// Массив с полученными числами
    	...
    	time = getTimeWithQP(array,j,sortType);
        
		// Поиск среднего времени выполнения сортировки
		sum = sum + time;
    ...
    // Подсчитываем среднее время сортировки
	avgMs = sum/i;

    // Печатаем результат
	printResult(sortType, j,avgMs);
}
